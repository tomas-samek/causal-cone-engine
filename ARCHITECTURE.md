...
| **Phase 2 — push** | Each active, non-debounced entity rewrites its outgoing edge deposits = own emission + pass-through of incoming, weighted by `edge_gamma × distance_factor × edge_atten`, with a directional bias for vacuum relays that scales with the *anisotropy* of what arrived (`incoming_dir` is Σ dir·density / Σ density, its length the anisotropy — not normalised: a relay lit evenly from opposite sides used to steer by a rounding-error residual and flip, and the frozen scene's lighting circled a period-4 limit cycle forever). Weights are then **renormalized** to sum to 1 — see the shadow note below. Parallelized with `rayon` (each entity owns a disjoint edge range). |
