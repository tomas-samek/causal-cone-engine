...
mod tests {
    ...
    fn tick_stays_inside_the_frame_budget() {
        ...
        for i in 0..12 {
            ...
            eprintln!("walking tick {:2}: {:7.2} ms  ({}{}hash {:.2} + relink {:.2} ms [footprints {:.2}, pipes {:.2}, front {:.2}, τ {:.2}, rest {:.2}], {} of {} pipes sent)",
                i, ms,
                if s.relinks > relinks_before { "RELINK, " } else { "" },
                if refreshed { "CROSS-LINK REFRESH, " } else { "" },
                s.hash_build_ms, s.relink_ms, s.relink_footprint_ms, s.relink_pipes_ms, s.relink_front_ms, s.relink_tau_ms,
                s.relink_ms - s.relink_footprint_ms - s.relink_pipes_ms - s.relink_front_ms - s.relink_tau_ms,
        ...
        for _ in 0..6 {
            ...
            let s = field.retina.stats;
            eprintln!("forced relink tick: {:.2} ms (hash {:.2} + retina relink {:.2} ms [footprints {:.2}, pipes {:.2}, front {:.2}, τ {:.2}, rest {:.2}])",
                ms, s.hash_build_ms, s.relink_ms, s.relink_footprint_ms, s.relink_pipes_ms, s.relink_front_ms, s.relink_tau_ms,
                s.relink_ms - s.relink_footprint_ms - s.relink_pipes_ms - s.relink_front_ms - s.relink_tau_ms);
